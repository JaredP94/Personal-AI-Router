...
for /f "delims=" %%V in ('jq -r --arg k "omlx-proxy"           ".components[$k]" "%VERSIONS_FILE%"')            do set "V_OMPROXY=%%V"
...
echo [3/14] Building omlx-proxy (v%V_OMPROXY%)...
cd /d "%ROOT%omlx-proxy"
go build -ldflags "-X main.Version=%V_OMPROXY%" -o omlx-proxy.exe . || goto :fail
echo       OK
...
copy /y "%ROOT%omlx-proxy\omlx-proxy.exe" "%BIN_OUT%\omlx-proxy.exe" >nul || goto :fail
...
echo  oMLX Proxy:       %BIN_OUT%\omlx-proxy.exe
