...
func TestCacheAffinitySSEPostDoneDataDoesNotRegister(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
	...
	context := prefixhash.Extract(http.MethodPost, "/v1/chat/completions", body)
	if _, ok := p.prt.Lookup("model", context.Full); ok {
		t.Fatal("SSE data after [DONE] registered a cache-affinity hint")
	}
}

func TestCacheAffinitySSEOversizedPostDoneDataDoesNotRegister(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
	...
	context := prefixhash.Extract(http.MethodPost, "/v1/chat/completions", body)
	if _, ok := p.prt.Lookup("model", context.Full); ok {
		t.Fatal("oversized SSE data after [DONE] registered a cache-affinity hint")
	}
}
