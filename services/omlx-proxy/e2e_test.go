...
// proxyBin is the real omlx-proxy binary, built once in TestMain so the
// e2e test exercises the shipped artifact (not just in-process handlers).
var proxyBin string

func TestMain(m *testing.M) {
	tmp, err := os.MkdirTemp("", "nvpair-omlxproxy-e2e-*")
	if err != nil {
		panic(err)
	}
	suffix := ""
	...
	proxyBin = filepath.Join(tmp, "omlx-proxy"+suffix)
	if out, err := exec.Command("go", "build", "-o", proxyBin, ".").CombinedOutput(); err != nil {
		panic("build omlx-proxy: " + err.Error() + "\n" + string(out))
	}
	code := m.Run()
	_ = os.RemoveAll(tmp)
	os.Exit(code)
...
// TestE2EFailoverOverRealBinary spawns the real omlx-proxy binary and
