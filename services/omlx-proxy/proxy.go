...
const (
	...
	// workload this proxy produces. This proxy only ever fronts oMLX.
	workloadEngine = "omlx"
)
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	// for om nodes; they arrive as discovery:nodes snapshots (handled in
	...
	slog.Debug("subscribing to discovery relay for routing targets", "service", string(noderec.ServiceOMLX))
	if err := p.codec.Notify(noderec.MethodSubscribe, noderec.SubscribeParams{Services: []noderec.ServiceKey{noderec.ServiceOMLX}}); err != nil {
...
func (retrySignal) Error() string { return "omlx-proxy: retry next candidate" }
...
// discovery:nodes snapshot: it projects every node advertising om with a dialable
...
func upstreamUnreachableID(nodeID string) string {
	return "omlx-proxy:upstream-unreachable:" + nodeID
}

// subscribedToNode projects a relay DirectoryNode onto the proxy's routable Node
// for the om service, returning false when the node doesn't advertise om or has
// no dialable address. The engine port comes from the om service key (the real
// oMLX port the broker's engine poller registered, not the proxy's listen
// port).
func subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	svc, ok := n.Services[noderec.ServiceOMLX]
	if !ok || n.IP == "" {
		return Node{}, false
	}
	...
	return Node{
		...
		// Filter on this node's oMLX models only, not the cross-engine union,
		// so a model a dual-engine node serves solely via Ollama isn't accepted as
		// an oMLX owner here (falls back to the union for a peer that sends
		// no attribution — see DirectoryNode.EngineModels).
		Models: append([]string(nil), n.EngineModels("omlx")...),
	}, true
}
